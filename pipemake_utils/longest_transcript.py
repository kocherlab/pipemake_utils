...
def seqParser():
    ...
    seq_parser.add_argument(
        "--database", help="Source database for the input file", required=True, choices=db_choices
    )
    seq_parser.add_argument("--input-filename", help="Input filename", required=True)
    seq_parser.add_argument("--output-prefix", help="Output filename prefix", required=True)
    ...
    seq_parser.add_argument("--output-type", help="Output type", choices=type_choices, default="AA")
...
def main():
    ...
    logging.basicConfig(
        filename=seq_args["output_prefix"] + ".log",
        level=logging.INFO,
    ...
    if not seq_args["input_type"]:
        ...
        seq_args["input_type"] = input_type_cvt[sniffSeqFile(seq_args["input_filename"])]
