...
def argParser ():
	...
	stat_parser.add_argument('--stat-col-int', help = 'Statistic column location', type = int)
...
def getChromInt (chrom_text, sep):
	...
	for split_pos, split_chrom in enumerate(chrom_text.split(sep)):
		...
		if not split_chrom.isdigit(): continue
			
		if chrom_int_pos: raise Exception (f"Multiple integer chromosome positions found in: {chrom_text}. Using separator: {sep}")

		chrom_int_pos = split_pos

	if not chrom_int_pos: raise Exception (f"Unable to determine the integer chromosome position from: {chrom_text}. Using separator: {sep}")
...
def main():
	...
	if plot_args['stat_col_int']:
		if not plot_args['plot_stat_text']: raise Exception ("--plot-stat-text must be specified if --stat-col-int is specified")
	...
	for num, (name, group) in enumerate(grouped_plot_dataframe):
		...
		group.plot(kind='scatter', x='ORDER', y=plot_stat_col, color=colors[num % len(colors)], ax=ax, s = 1, zorder = 2)
	...
	ax.set_ylim([plot_dataframe[plot_stat_col].min() * 1.5, plot_dataframe[plot_stat_col].max() * 1.5])
